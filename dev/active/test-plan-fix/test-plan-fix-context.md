...
修復 P0 測試套件，將測試通過率從 12.5% (2/16) 提升到至少 75% (12/16)，理想目標 87.5%+ (14+/16)。
...
```mermaid
graph TD
    A[發現測試失敗] --> B{修復 selector/timing 後仍失敗?}
    B -->|否| C[修復成功]
    B -->|是| D{手動測試能通過?}
    D -->|是| E[測試代碼問題 - 繼續修復]
    D -->|否| F[功能 Bug]
    F --> G[記錄詳細 Bug Report]
    G --> H[標記測試為 skip/known-issue]
    H --> I[更新 KNOWN_ISSUES_SOLUTIONS.md]
    I --> J[繼續修復下一個測試]
```
...
|------|---------|---------|---------|---------|---------|------|
| Phase 1 | 15:00 | 16:00 | ________ | ________ | ________ | ⏳ Pending |
| Phase 2 | 16:00 | 16:40 | ________ | ________ | ________ | ⏳ Pending |
