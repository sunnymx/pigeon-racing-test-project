...
| #06 | 軌跡詳情 - 航點列表 | 核心 | P0 | 14 | ✅ 完成 |
| **輔助功能** ||||||
| #07 | 多選鴿子顯示 | 進階 | P1 | 5 | ✅ 完成 |
| #08 | 鴿舍列表 Tab | 進階 | P1 | 5 | ✅ 完成 |
...
| 項目 | 狀態 |
|------|------|
| 測試檔案 | `tests/e2e/tc-06-001-trajectory-details.spec.ts` |
| Helper | `tests/helpers/trajectory-details.ts`, `tests/helpers/fixtures.ts` (setupTrajectoryDetails) |
| 測試數量 | 14 個 (6.1~6.15，排除 6.5) |
| 本地測試 | ✅ 14/14 passed |
| CI 測試 | ✅ 14/14 passed |
| 完成日期 | 2025-12-12 |
...
- **終點航點解析**: DOM 結構為 7 元素（🏁 + 航點號 + 5 數據），需特殊處理
- **數值範圍調整**: 實測數據超出原規格範圍，已調整（分速 500~3000、高度 0~2000）
- **Test 6.5**: 前往功能暫未實作
...
| 項目 | 狀態 |
|------|------|
| 測試檔案 | `tests/e2e/tc-07-001-multi-select.spec.ts` |
...
- **載入等待策略**: 等待「載入資料中」提示消失後再驗證軌跡
- **2D 偏好失效處理**: 進入軌跡後檢測模式，若為 3D 則切換到 2D
- **Test 7.2**: 顏色區分使用替代驗證（確認軌跡存在即可）
...
| 項目 | 狀態 |
|------|------|
| 測試檔案 | `tests/e2e/tc-08-001-loft-list.spec.ts` |
| Helper | `tests/helpers/loft-list.ts` |
| 測試數量 | 5 個 (8.1~8.5) |
| 本地測試 | ✅ 5/5 passed |
| CI 測試 | ✅ 5/5 passed |
| 完成日期 | 2025-12-12 |
...
- **鴿舍選擇器**: 使用原生 HTML `<select>` 元素（非 Angular Material mat-select）
- **選項存取**: 直接使用 `option:not([disabled])` 選擇器，無需點擊展開
- **選項值**: 鴿舍 ID（數字格式），非鴿舍名稱
- **2D 偏好失效處理**: 查看軌跡後檢測模式，若為 3D 則切換到 2D
...
| 項目 | 狀態 |
|------|------|
| 測試檔案 | `tests/e2e/tc-09-001-checkbox-panel.spec.ts` |
| Helper | `tests/helpers/checkbox-panel.ts` |
| 測試數量 | 5 個 (9.1~9.5) |
| 本地測試 | ✅ 5/5 passed |
| CI 測試 | ✅ 5/5 passed (7.5m) |
| 完成日期 | 2025-12-13 |
...
| 項目 | 狀態 |
|------|------|
| 測試檔案 | `tests/e2e/tc-10-001-radar-mode.spec.ts` |
| Helper | `tests/helpers/radar-mode.ts` |
| 測試數量 | 5 個 (10.1~10.5，簡化自原 8 個) |
| 本地測試 | ✅ 5/5 passed (1.4m) |
| CI 測試 | ✅ 5/5 passed |
| 完成日期 | 2025-12-13 |

#### 開發備註

- **載入時間**: 雷達模式需下載整場賽事軌跡，載入時間約 1-5 分鐘
- **超時設定**: 本地 4 分鐘 (240s)，CI 5 分鐘 (300s)，測試超時使用 6-8 倍乘數
- **三階段等待策略**:
  1. 等待下載對話框消失（包含百分比進度）
  2. 等待「载入资料中」提示消失
  3. 等待播放按鈕 (play_arrow) 出現
- **雷達模式卡片識別**: 使用「最终名次」文字作為區分標識
...
| # | 檢查項目 | 優先級 | 驗證方式 | 狀態 |
|---|---------|--------|---------|------|
| 10.1 | 對話框交互 | P2 | 對話框顯示、取消返回、確定按鈕 | ✅ |
| 10.2 | 進入雷達模式 | P2 | 下載進度、介面載入 | ✅ |
| 10.3 | 介面元素驗證 | P2 | 鴿子卡片列表、里程碑統計 | ✅ |
| 10.4 | 搜尋功能 | P2 | 輸入環號 → 結果過濾 | ✅ |
| 10.5 | 播放控制 | P2 | 播放/暫停按鈕正常 | ✅ |

#### 原始測試項目（已整合）

| 原始 # | 項目 | 整合至 |
|--------|------|--------|
| 10.1 | 確認對話框 | → 10.1 對話框交互 |
| 10.2 | 取消功能 | → 10.1 對話框交互 |
| 10.3 | 下載進度 | → 10.2 進入雷達模式 |
| 10.4 | 介面載入 | → 10.2 進入雷達模式 |
| 10.5 | 鴿子列表 | → 10.3 介面元素驗證 |
| 10.6 | 里程碑統計 | → 10.3 介面元素驗證 |
| 10.7 | 搜尋功能 | → 10.4 搜尋功能 |
| 10.8 | 播放控制 | → 10.5 播放控制 |
...
- V2.6 (2025-12-12): 記錄點 #05 (3D 模式) 測試開發完成
- V2.7 (2025-12-12): 記錄點 #06 (軌跡詳情) 測試開發完成，14 個測試案例
- V2.8 (2025-12-12): 記錄點 #07 (多選鴿子顯示) 測試開發完成，5 個測試案例
